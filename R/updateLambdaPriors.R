updateLambdaPriors = function(Lambda,Delta, rL){
   ...
   for(r in seq_len(nr)){
      ...
      if(is.matrix(lambda)){
         ...
         ncr = dim(lambda)[3]
         bPsi = nu/2 + 0.5*lambda2 * array(tau,c(nf,1,ncr))[,rep(1,ns),]
         psi = array(rgamma(nf*ns*ncr, aPsi, bPsi), c(nf,ns,ncr))
         M = psi * lambda2
         ad = a1 + 0.5*ns*nf
         bd = b1 + 0.5 * colSums(tau * apply(M,c(1,3),sum)) / delta[1,]
         delta[1,] = rgamma(ncr,ad,bd)
         for(h in 1+seq(nf-1)){
            tau = apply(delta,2,cumprod)
            ad = a2 + 0.5*ns*(nf-h+1)
            bd = b2 + 0.5 * colSums(tau[-c(1:(h-1)),] * apply(M[-c(1:(h-1)),,,drop=FALSE],c(1,3),sum)) / delta[h,]
