#' @importFrom stats runif rnorm rgamma dt
...
updateLatentLoadingOrder = function(eta,lambda,alpha,delta, rL){
   ...
   for(h in 1:(nf-1)){
      ...
      tmp = array(exp(tauLogMat[,,h:(h+1)]), c(N,N,2,ns))
      ...
      log_prior_loading = log_prior_delta+log_prior_Lambda
      log_prior_loading_swap = log_prior_delta+log_prior_Lambda_swap
