...
plotGradient=function (hM, Gradient, pred, measure, index = 1, prob = c(0.025, 0.5, 0.975),...)
{
   ...
   if (is.factor(xx)) {
      ...
      ggplot(toPlot, aes(x = xx, y = me), fill = supp) + geom_bar(position = position_dodge(),
                                                                  stat = "identity") + xlab(xlabel) + ylab(ylabel) +
         geom_errorbar(aes(ymin = lo, ymax = hi), width = 0.2,
                       position = position_dodge(0.9))
   }
   else {
      plot(xx, qpred[2, ], ylim = c(min(qpred[1, ]), max(qpred[3,
                                                               ])), type = "l", xlab = xlabel, ylab = ylabel, ...)
