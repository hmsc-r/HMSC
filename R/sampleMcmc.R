...
#' @param saveToDisk whether to save replicates to the disk once they 
#'   are ready and discard them from RAM memory
#' @param verbose
#' @param adaptNf
#' @param nChains number of mcmc chains
#' @param nCores number of cores (max of one core per chain)
#' @param dataParList
#' @param updater
#'
#' @examples
...
sampleMcmc = function(samples, transient=0, thin=1, initPar=NULL, 
	repN=1, saveToDisk=FALSE, verbose=samples*thin/100, adaptNf=NULL, 
	nChains=1, nCores=1, dataParList=NULL, updater=list()){
		...
		for(repN in 1:repN){
		   ...
		   for(iter in 1:(transient+samples*thin)){
		      if(!identical(updater$Gamma2, FALSE))
		         Gamma = updateGamma2(Z=Z,Gamma=Gamma,iV=iV,
							iSigma=iSigma,Eta=Eta,Lambda=Lambda, X=X,Pi=Pi,
							Tr=Tr,C=C, iQg=iQg, mGamma=mGamma,iUGamma=iUGamma)

		      if(!identical(updater$BetaLambda, FALSE)){
		         BetaLambdaList = updateBetaLambda(Y=Y, Z=Z, Gamma=Gamma, iV=iV, 
							iSigma=iSigma, Eta=Eta,Psi=Psi,Delta=Delta,rho=rho, 
							iQg=iQg, X=X,Tr=Tr,Pi=Pi,C=C)
		      ...
		      if(!identical(updater$InvSigma, FALSE))
		         iSigma = updateInvSigma(Y=Y,Z=Z,Beta=Beta,iSigma=iSigma,Eta=Eta,
							Lambda=Lambda, distr=distr,X=X,Pi=Pi, aSigma=aSigma,bSigma=bSigma)

		      if(!identical(updater$Z, FALSE))
		         Z = updateZ(Y=Y,Z=Z,Beta=Beta,iSigma=iSigma,
							Eta=Eta,Lambda=Lambda, X=X,Pi=Pi,distr=distr)

		      for(r in seq_len(nr)){
		         if( (is.list(adaptNf) && adaptNf[[r]][1] >= repN && adaptNf[[r]][2] >= iter) || (is.vector(adaptNf) && adaptNf[r] >= iter)){
		            listPar = updateNf(eta=Eta[[r]],lambda=Lambda[[r]], 
		      ...
		      if((iter>transient) && ((iter-transient) %% thin == 0)){
		         postList[[(iter-transient)/thin]] = private$combineParameters(Beta=Beta, 
		      ...
		      if((verbose > 0) && (iter%%verbose == 0)){
		         ...
		         print(sprintf("Chain %d, replicate %d, iteration %d of %d, (%s)", chain, 
								repN, iter, transient+samples*thin, samplingStatusString))
		      }
		   }
		   repList[[repN]] = postList
		}
		return(list(repList, postList))
	}


	if(nCores > 1){
		cl <- parallel::makeCluster(nCores, type = "MPI")
		parallel::clusterExport(cl, list(parList, Y))
	  tmp <- parallel::clusterApplyLB(cl, initSeed, fun=runChain)
		self$repList <- lapply(tmp, function(x){return(x[[1]])})
		self$postList <- lapply(tmp, function(x){return(x[[2]])})
	}else{
		tmp <- runChain(initSeed)
		self$postList <- tmp$postList
		self$repList <- tmp$repList
	}
