...
computePredictedValues = function(hM, partition=NULL, start=1, Yc=NULL, mcmcStep=1, expected=TRUE, nCores=1){
   if(is.null(partition)){
      ...
      for (k in 1:nfolds){
         ...
         hM1 = sampleMcmc(hM1, samples=hM$samples, thin=hM$thin, transient=hM$transient, adaptNf=hM$adaptNf, nChains=length(hM$postList), nCores=nCores)
