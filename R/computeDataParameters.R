...
computeDataParameters = function(hM,rLKronLogDetList=NULL){
   tfla = tf$linalg
   tfm = tf$math
   ic = function(...){
      return(as.integer(c(...)))
   ...
   for(r in seq_len(hM$nr)){
      ...
      if(inherits(hM$rL[[r]],"HmscSpatialRandomLevel",TRUE)==1){
         ...
         for(l in seq_len(length(hM$rL[[r]]$rLList))){
            ...
            if(rL$sDim > 0){
               ...
               if(rL$spatialMethod=="Full"){
                  ...
                  if(hM$rL[[r]]$etaMethod=="TF" || hM$rL[[r]]$etaMethod=="TF_krylov" || hM$rL[[r]]$alphaMarginalMethod=="TF_obs" ||
                     hM$rL[[r]]$alphaMethod=="TF_full" || hM$rL[[r]]$alphaMethod=="TF_direct_krylov" || hM$rL[[r]]$alphaMethod=="TF_direct_local_krylov"){
                     WStack = Wg
                     if(l==1){
         ...
         gc()
         if(!is.null(rLKronLogDetList) && !is.null(rLKronLogDetList[[r]])){
            rLPar[[r]]$logDetK = rLKronLogDetList[[r]]
         } else{
            if(hM$rL[[r]]$alphaMethod=="TF_direct_krylov" || hM$rL[[r]]$alphaMethod=="TF_direct_local_krylov"){
               ...
               if(batchSize==0){
                  ...
                  tmp_fun = function(batch){
                     ...
                     if(is.null(L)) L<<-tf$Variable(tfla$cholesky(A)) else L$assign(tfla$cholesky(A))
