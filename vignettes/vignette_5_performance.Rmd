...
We ran the `r nChains` chains in parallel using the option `nParallel
= nChains`, and recorded the computational time for MCMC sampling
using a multi-core computer (in this case with
`r parallel::detectCores()` cores).
The timings and relative timings are very strongly dependent on the
system and its load, including external resources such as LAPACK
libraries for numerical matrix algebra.
