...
author: "Gleb Tikhonov, Øystein H. Opedal, Nerea Abrego, Aleksi Lehikoinen & Otso Ovaskainen"
date: "14 March 2019"
output:
...
We also loaded the library corrplot which will be used for plotting, and we set the random number seed to make the results presented here reproducible. This time we did not choose `set.seed(1)` but instead `set.seed(8)` as that results in a simulated phylogeny that fits better our purpose.
...
Here the parameter `rho` sets the extent to which the traits are phylogenetically constrained. With `rho` equalling one, the traits would be distributed exactly following the diffusion (Brownian motion) model of trait evolution. Thus, `1-rho` represent species-specific independent residual variation.
...
```{r V, fig.height=6, fig.width=6, fig.cap="\\label{fig: I} Phylogenetic patterns of species' trait values"}
...
The reason why we did not assume that all species pairs belonging to the same body size class would exhibit a strong negative correlation with each other is that this would be logically impossible. Thinking in terms of species occurrence, assume that we would like to implement a strong avoidance among species A, B and C. We could start by assuming that the species A and B exhibit competitive exclusion and that they do not occupy the same sampling units, so that a sampling unit is occupied either by species A or by species B. We could likewise assume that also the species B and C exhibit competitive exclusion and thus a given sampling unit is occupied either by species B or by species C. But these two assumptions lead to the situation where the species A and C will necessarily be found in the same sampling units, because they both would be found in those sampling units where species B is not present. Thus, it would not be possible to assume further that due to competitive exclusion between species A and C, these two would also occupy different sampling units. While we could modify this example in many ways to avoid the problem (e.g., one sampling unit can be occupied only by one of the species A, B and C), it makes the point that pair-wise associations among species need to be compatible with each other.

In the above script, we ensure that the smallest eigenvalue of the correlation matrix `W` is positive, i.e. we ensure that matrix `W` remains positive definite. The condition of positive definiteness corresponds to the intuitive assumption that the associations must be logically feasible, in the sense discussed above. In the above script, the more negative the correlation `rho` between a competing species pair is assumed to be, the smaller will be the number of species pairs that can show that negative correlation with each other. We note that the situation is not identical with respect to positive associations. For example, it can be that some sampling units are occupied by all species whereas in other sampling units none of the species are present, creating a strong positive association among all species pairs.
...
```{r XIII, fig.height=7, fig.width=6, fig.cap="\\label{fig: XIII} Histograms of effective sample sizes and potential scale reduction factors (psrf) for Beta, Gamma, and Omega parameters"}
...
```{r XIV, fig.height=4, fig.width=4, fig.cap="\\label{fig: XIV} Histogram of species-specific explanatory r2 values"}
...
The design matrix `X` has been derived from `XData` and `XFormula` when constructing the HMSC object. Following the usual conventions of building generalized linear models, the design matrix includes the intercept by default. The categorical explanatory variables are expanded into dummy variables indicating which level is present in the focal sampling unit. Here the second column corresponds to the explanatory variable habitat, and a value 1 means that the habitat is open habitat. To avoid confounding with the intercept, one of the levels is left as the reference level, which is why there is no column indicating when the habitat is forest habitat (this is the case when habitat is not in habitat). The continuous covariate climate is present here in two columns modelling its linear and quadratic effects, respectively.
...
```{r XVI, fig.height=5, fig.width=7, fig.cap="\\label{fig: XVI} Variance partitioning for each of 50 species"}
...
```{r XIX, fig.height=5, fig.width=5, fig.cap="\\label{fig: XIX} Heatmap showing species residual associations"}
...
We may compare these estimated associations to those that we assumed in generating data. Thus we use `corrplot` to plot the matrix `W` showing which species pairs were assumed to compete with each other.

```{r XX, fig.height=5, fig.width=5, fig.cap="\\label{fig: XX} Heatmap showing species associations assumed when generating data"}
rownames(W) = spnames
colnames(W) = spnames
...
We observe that the assumed (`W`) and estimated (`Omega`) matrices are consistent in the sense that both contain a small number of negative associations, while the majority of the species pairs have no association. To check how often the estimated negative associations match with the same species pairs for 
...
kable(ma, caption = "Assumed and observed species associations")
```
...
Sometimes the values of the primary parameters `Beta` and `Gamma` can be difficult to interpret, especially if there are categorical traits or environmental covariates in which case their effects are coded through dummy variables. One alternative for visualizing the results is to plot how the community changes over some environmental gradient of interest. This can be done in HMSC by constructing environmental gradients with the function `constructGradient`, then predicting communities over those gradients by the function `predict`, and finally using `plotGradient`to visualize the predicted variation. We consider first a climatic gradient.
...
```{r XXIV, fig.height=4, fig.width=5, fig.cap="\\label{fig: XXIV} Effect of climate on total abundance"}
...
```{r XXV, fig.height=4, fig.width=5, fig.cap="\\label{fig: XXV} Effect of climate on the abundance of species 1"}
plotGradient(m, Gradient, pred=predY, measure="Y", index = 1, showData = TRUE)
```

This figure shows that `species_089` is most abundant at relatively cool climates. Finally, by selecting `measure="T"` we can visualize how community-weighted mean trait values behave over the environmental gradient. Now `index` selects the trait (as ordered in the matrix `m$Tr`).

```{r XXVI, fig.height=4, fig.width=5, fig.cap="\\label{fig: XXVI} Effect of climate on community-weighted mean thermal optimum"}
...
As habitat is a categorical variable, the gradient involves only two sampling units, one belonging to forest and the other one to open habitat. We have decided to normalize the climatic variable to its overall mean in the data (for other options, see the F1-help for `constructGradient`).
...
```{r XXVIII, fig.height=4, fig.width=5, fig.cap="\\label{fig: XXVIII} Effect of habitat on the abundance of the most habitat-responsive species"}
predY = predict(m, XData=Gradient$XDataNew, studyDesign=Gradient$studyDesignNew,
                ranLevels=Gradient$rLNew, expected=TRUE)
...
As expected, the species shows a higher predicted response to forests than to open habitats, a pattern visible also in the raw data. We note that as habitat is a categorical covariate, now the environmental gradient is more naturally presented in terms of a boxplot. We further note that we have set `jigger = 0.2` to randomly move the observed data (the dots) in the horizontal direction and thus to avoid overlapping points.
...
```{r XXIX, fig.height=4, fig.width=5, fig.cap="\\label{fig: XXIX} Effect of habitat on community-weighted mean habitat use"}
...
```{r XXX, fig.height=4, fig.width=4, fig.cap="\\label{fig: XXX} Histogram of species-specific r2 values"}
...
```{r XXXI, fig.height=5, fig.width=7, fig.cap="\\label{fig: XXXI} Variance partitioning for each of 50 species"}
...
```{r XXXII, fig.height=5, fig.width=5, fig.cap="\\label{fig: XXXII} Heatmap showing species residual associations"}
OmegaCor = computeAssociations(m)
supportLevel = 0.95
toPlot = ((OmegaCor[[1]]$support>supportLevel) 
          + (OmegaCor[[1]]$support<(1-supportLevel))>0)*OmegaCor[[1]]$mean
...
Let us next assume that we would have trait data only on habitat use, but we would lack information on species fecundities and thermal optima. We repeat the above analyses with a model modified to correspond to this assumption.

```{r XXXIII, fig.height=4, fig.width=4, fig.cap="\\label{fig: XXXIII} Variance partitioning for each of 50 species"}
...
```{r XXXIV, fig.height=5, fig.width=7, fig.cap="\\label{fig: XXXIV} Variance partitioning for each of 50 species"}
VP = computeVariancePartitioning(m, group = c(1,1,2,2),groupnames=c("habitat","climate"))
plotVariancePartitioning(m, VP = VP)
```

This model is very similar to the full model in terms of the explanatory power, as well as in terms of the variance partitioning. However, now the traits explain a much smaller part ($R^2$ = `r round(100*VP$traitR2)`%) of the variation in the species niches than was the case for the full model. This is because we are missing some relevant traits that would have explained the remaining part of the variation in species niches.
...
The parameter estimate is now higher than in the full model. This is the case because now residual variation in species niches (not explained by the traits) include a phylogenetic signal generated by the phylogenetic structure of the missing traits.
