...
We also loaded the library corrplot which will be used for plotting, and we set the random number seed with `set.seed(1)` to make the results presented here reproducible.
...
We will next generate simulated trait values for three traits for each of the `r ns` species. For the sake of illustration we call these traits 'habitat preference for forest' (trait 1, to be abbreviated as H) and 'thermal optimum' (trait 2, to be abbreviated as T).
...
As expected, both traits show a clear phylogenetic signal, i.e. related species show similar trait values.
...
The next step is to define the species niches, i.e. how the species abundances depend on the match between their traits and the environmental conditions. In the script below, we assume that the trait 'habitat use' measures the preference of the species to forests, so that for forest habitats we add this trait value to the linear predictor whereas for open habitats we subtract it. We further add the term $-(C_i-T_j)^2/4$ where $C_i$ is the climatic condition at location $i$ and $T_j$ is the thermal optimum of the species $j$. This makes the linear predictor and hence species abundances the largest when the species thermal optimum matches with the climatic conditions. We can write these choices as the equation  $E[L_{ij}]=H_j F_i - H_j (1-F_i)-(C_i-T_j)^2/4$. Here $E[L_{ij}]$ is the expected value of the linear predictor, $H_j$ is the habitat use trait of the species $j$, $F_i=1$ if location $i$ belongs to forest and $F_i=0$ if it belongs to open area. To see how this equation is implemented in the script below, we note that
...
The traits explain a high amount of the variation in the species niches. This is because we have included in the HMSC model those traits that truly matter in how species respond to the environmental variation. We next ask how the influence of traits on species niches propagates into the influence of traits in species abundances.

```{r}
VP$R2T$Y
```

The traits explain a high amount of variation also in species abundances.
...
Expectedly, the random effects play now a bigger role. Let us plot the structure of the species associations, as estimated by the random effect.
...
Now the traits explain a smaller part of abundance variation than was the case for the full model. This is because we are missing some relevant traits that would have explained the remaining part.
