...
rL.spatial = setPriors(rL.spatial,nfMin=1,nfMax=1) #We limit the model to two latent variables for visualization purposes
...
```{r}
# alpha = 1/2
# val = xycoords%*%c(cos(alpha),sin(alpha))
# yprob = yprob[order(val),]
# XData = data.frame(x1 = XData[order(val),])
# xycoords = xycoords[order(val),]
...
```{r, fig.height=4, fig.width=7, fig.cap="\\label{fig: III} Posterior trace plot of the spatial scale parameter of the nngp spatial model."}
m.gpp = convertToCodaObject(m.gpp)
plot(m.gpp$Alpha[[1]])
summary(m.gpp$Alpha[[1]])
```
