...
We also loaded the library corrplot which will be used for plotting, and we set the random number seed to make the results presented here reproducible.
...
To assess the model's explanatory power, we apply the `evaluateModelFit` function to the posterior predictive distribution computed by the function `computePredictedValues`.
...
Reassuringly, these estimates are consistent with those estimated by a model without the random effect.
...
In general, residual associations among species can be generated by correlated responses to missing covariates, or by ecological interactions. While HMSC analyses can be used to identify residual associations among the species, they cannot be used to separate whether the associations are due to missing covariates or due to ecological interactions. This is not a limitation of HMSC *per se*, but a limitation of the type of data that is used as input: identical data on species occurrences or abundances can be generated either by responses to missing covariates or by ecological interactions. To ascertain whether the associations are due to missing covariates or ecological interactions, some other type of data would thus be needed. As one example, one could conduct experiments where the fitnesses of the species are measured when they are or are not allowed to interact with each other. In the absence of such additional data, the best that can be done is to control in the model for those environmental covariates that can be expected (or are shown by cross-validation) to be relevant, and then interpret the remaining residual associations with caution, and keeping the ecological knowledge about the study organisms in mind.
...
We now extract the posterior means of the $\eta$ and $\lambda$ parameters, which represent the site loadings ($\eta$) and species loadings ($\lambda$) on the latent factors. We then assign colors to the site loadings according to the value of the covariate x2 used in simulating the data (but that was not used in model fitting).

```{r E2, include=F}
#DEFINING THE biPlot FUNCTION
biPlot=function(hM, etaPost, lambdaPost, factors=c(1,2), colVar="x1"){
         if(colVar!="n"){
...
```{r E3, fig.height=5, fig.width =5}
etaPost=getPostEstimate(m, "Eta")
lambdaPost=getPostEstimate(m, "Lambda")

biPlot(m, etaPost = etaPost, lambdaPost = lambdaPost, factors = c(1,2), colVar = "x2")
```

# Non-linear model for a community of four species
