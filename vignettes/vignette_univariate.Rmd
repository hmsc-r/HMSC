...
```{r}
m = sampleMcmc(m, thin = 2, samples = 1000, transient = 500,
                    nChains = 2, verbose = 500)
```

Here we have selected some parameters (`thin`, `samples`, `transient`, `nChains`) that control how the parameters are estimated (more precisely, how the posterior distribution is sampled). We will ignore these for a moment but return to their meaning soon. The main thing is that now the model object `m` also included estimated parameters, in the same way as the object `m.lm` includes the parameters estimated by the `lm` function.
...
```{r}
preds.hmsc.posterior = computePredictedValues(m)
evaluateModelFit(hM=m, predY=preds.hmsc.posterior)
```
...
effectiveSize(mpost$Beta)
gelman.diag(mpost$Beta,multivariate=FALSE)$psrf
preds.hmsc.posterior = computePredictedValues(m, expected = FALSE)
evaluateModelFit(hM=m, predY=preds.hmsc.posterior)
```
...
We first consider a hierarhical study design in which 100 sampling units belong to a discrete set of `np=10`plots. We assume that the plots have an additive effect to the response variable.
...
With the maximum likelihood framework, one standard option for fitting a mixed model to these data would be to apply the `lmer`function as `lmer(y~x+(1|plot.id),data=da)`. Fitting a similar model with HMSC can be done as follows. 
...
m = sampleMcmc(m, thin = 1, samples = 1000, transient = 500,
                    nChains = 1, verbose = 500)
```

Here we have included two new input parameters. First, `studyDesign` defines the nature of the study design, here the individuals sampling units and and the plots to which the sampling units belong to. Second, `ranLevels` includes a list of those structures of the study design to which a random effect will be included, here the plot. The object `rL` was created with the function `HmscRandomLevel` assuming the default options except that with `rL$nfMin = 1`and `rL$nfMax = 1` we have fixed the number of the underlying latent factors to one. This is because we assume a univariate model, and thus it is not meaningful to have several latent variables. In usual applications of HMSC, the data are multivariate, and the latent factor approach is used to reduce the dimensionality by having a much smaller number of latent factors than what is the number of species.

Note that we have decreased here the MCMC sampling by including only a single chain with thinning of 1. This is not at all recommended, as it prevents comparison among chains and compromises the reliability of the results. We have done so only to make the markdown file behind the vignette to evaluate fast so that the reader can reproduce the results without too much waiting.
...
What is predicted here are the same data that were used to fit the model, and thus $R^2$ measures explanatory power rather than predictive power. If we would add more predictors, we could make $R^2$ approach to one even if the predictors would represent just random noise, which phenomenon is known as overfitting. For this reason, the preditive power of the model should be evaluated more critically by cross-validation. To apply cross-validation, we first assing the samples into a number of folds. For example, we may apply two-fold cross-validation across the samples by making the following partition
...
The idea of cross-validation is that when making predictions for a particular fold, data from that fold is not used for parameter estimation to avoid good model fit due to overfitting. Model fitting and predictions are made separately for each fold, so that in the end a prediction is obtained for each sampling unit.
...
As expected, the cross-validation based predictive $R^2$ is lower than the explanatory $R^2$. However, it is still relatively high, meaning that the model has substantial power to predict the value of the response variable $y$ in a sampling unit not seen when when fitting the model to training data.

Making the two-fold cross-validation required fitting the model twice, and more generally performing an k-fold cross-validation requires fitting the model k times, s it can be computationally intensive. For this reason, we have applied here two-fold cross-validation rather than e.g. leave-one-out cross-validation. Increasing the number of folds means that more data are available for fitting the model, which can be expected to lead to greater predictive performance. In this sense, the predictive power estimated by two-fold cross-validation is likely to underestimate the true predictive power of the model when fitted to all data.

Cross-validation can be made in many ways, and how exactly it should be done depends on which aspect of predictive power one wishes to measure. To illustrate, let us partition the plots rather than the sampling units into different folds.
...
As seen by examining the correspondence between plots and the partition (shown here for the first 15 samplig units), now all sampling units for a certain plot have been included in the same fold.
...
MF$R2
```
The predictive power of the model is smaller when assigning entire plots rather than individual sampling units into folds. This is because the prediction task is now more difficult: when making a prediction for a particular sampling unit, the model was now fitted without including data from any other sampling units from the same plot. Thus it has no possibility to estimate the actual random effect for the focal plot, and thus now predictive power is based solely on the fixed effects.
...
```{r}
   ...
   a = mvrnorm(mu=rep(0,n),Sigma = sigma.spatial*exp(-as.matrix(dist(xycoords))/alpha.spatial))
   L = L + a
   y = L + a + rnorm(n, sd = sigma)
...
The spatial structure are visible in the data by nearby sampling units having similar responses `y`, even if the predictor `x` is not (due to our assumptions) spatially autocorrelated.
...
rL = HmscRandomLevel(sData = as.data.frame(xycoords))
rL$nfMin = 1
rL$nfMax = 1
...
As the model includes a spatially structured random effects, its predictive power is based on both the fixed and the random effects. Concerning the latter, the model can utilize observed data from nearby sampling units included in model fitting when predicting the response for a focal sampling unit that is not included in model fitting.
