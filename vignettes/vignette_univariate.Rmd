...
author: Gleb Tikhonov, Ã˜ystein H. Opedal, Nerea Abrego, Aleksi Lehikoinen & Otso
  Ovaskainen
date: "14 March 2019"
output:
...
To look at the parameter estimates, we may extract the posterior distribution from the model object and convert it into a `coda` object. 
...
We note that we have now set `verbose = 0` to suppress the output on progress of MSMC sampling. Generally we recommend the user not to set `verbose = 0` to see that MCMC sampling is moving as it should and to be able to estimate how long it will take before the sampling finishes. We have suppressed the output just to make the vignette a bit more compact.
...
We note that the MCCM mixing for the lognormal Poisson model is better than for the baseline Poisson model, which is a positive feature, as we expect the lognormal Poisson model to be generally more applicable to ecological data than the baseline Poisson model. As illustrated above, the same types of output can be given for the lognormal Poisson model as for the standard Poisson model.
...
Model fitting and evaluation of explanatory and predictive powers can be done as before.
